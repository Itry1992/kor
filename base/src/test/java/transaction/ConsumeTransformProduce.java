...
//许多基于Kafka的应用，尤其是Kafka Stream应用中同时包含Consumer和Producer，
// 前者负责从Kafka中获取消息，后者负责将处理完的数据写回Kafka的其它Topic中。


//为了实现该场景下的事务的原子性，
// Kafka需要保证对Consumer Offset的Commit与Producer对发送消息的Commit包含在同一个事务中。
// 否则，如果在二者Commit中间发生异常 kafka 通过 sendOffsetsToTransaction 来保证了该功能的实现
public class ConsumeTransformProduce {
    private static Properties getProducerProps() {
        ...
        props.put("client.id", "producer-syn-2"); // 发送端id
    ...
    private static Properties getConsumerProps() {
        ...
        props.put("client.id", "producer-syn-1"); // 发送端id
    ...
    public static void main(String[] args) {
        ...
        // 订阅主题
        consumer.subscribe(Collections.singletonList("producer-syn"));
        for (int i = 0;i<100 ;i++ ) {
            // 开启事务
            ...
            try {
                ...
                // 提交偏移量
                producer.sendOffsetsToTransaction(commits, new ConsumerGroupMetadata("test_3"));
                // 事务提交
