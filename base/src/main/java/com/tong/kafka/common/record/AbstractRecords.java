...
import com.tong.kafka.common.utils.AbstractIterator;
...
public abstract class AbstractRecords implements Records {
    ...
    public static int estimateSizeInBytes(byte magic,
        ...
        if (magic <= RecordBatch.MAGIC_VALUE_V1) {
            for (Record record : records)
                size += LOG_OVERHEAD + LegacyRecord.recordSize(magic, record.key(), record.value());
        } else {
    ...
    public static int estimateSizeInBytesUpperBound(byte magic, CompressionType compressionType, ByteBuffer key,
        ...
        if (magic >= RecordBatch.MAGIC_VALUE_V2)
            ...
            return LOG_OVERHEAD + LegacyRecord.recordOverhead(magic) + LegacyRecord.recordSize(magic, key, value);
        else
            return LOG_OVERHEAD + LegacyRecord.recordSize(magic, key, value);
    }
    ...
    public static int recordBatchHeaderSizeInBytes(byte magic, CompressionType compressionType) {
        if (magic > RecordBatch.MAGIC_VALUE_V1) {
            ...
            return LOG_OVERHEAD + LegacyRecord.recordOverhead(magic);
        } else {
            return 0;
        }
    }
