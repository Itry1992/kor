...
import com.tong.kafka.common.message.OffsetFetchRequestData;
...
public class OffsetFetchRequest extends AbstractRequest {
    ...
    public static class Builder extends AbstractRequest.Builder<OffsetFetchRequest> {
        ...
        public Builder(String groupId,
            ...
            if (partitions != null) {
                Map<String, OffsetFetchRequestData.OffsetFetchRequestTopic> offsetFetchRequestTopicMap = new HashMap<>();
                for (TopicPartition topicPartition : partitions) {
                    String topicName = topicPartition.topic();
                    OffsetFetchRequestData.OffsetFetchRequestTopic topic = offsetFetchRequestTopicMap.getOrDefault(
                        topicName, new OffsetFetchRequestData.OffsetFetchRequestTopic().setName(topicName));
        ...
        public Builder(Map<String, List<TopicPartition>> groupIdToTopicPartitionMap,
            ...
            List<OffsetFetchRequestData.OffsetFetchRequestGroup> groups = new ArrayList<>();
            for (Entry<String, List<TopicPartition>> entry : groupIdToTopicPartitionMap.entrySet()) {
                ...
                if (tpList != null) {
                    Map<String, OffsetFetchRequestData.OffsetFetchRequestTopics> offsetFetchRequestTopicMap =
                        new HashMap<>();
                    for (TopicPartition topicPartition : tpList) {
                        String topicName = topicPartition.topic();
                        OffsetFetchRequestData.OffsetFetchRequestTopics topic = offsetFetchRequestTopicMap.getOrDefault(
                            topicName, new OffsetFetchRequestData.OffsetFetchRequestTopics().setName(topicName));
                        topic.partitionIndexes().add(topicPartition.partition());
                        offsetFetchRequestTopicMap.put(topicName, topic);
                    }
                    topics = new ArrayList<>(offsetFetchRequestTopicMap.values());
        ...
        public OffsetFetchRequest build(short version) {
            ...
            if (version < 8) {
                ...
                if (!data.groups().isEmpty()) {
                    OffsetFetchRequestData.OffsetFetchRequestGroup group = data.groups().get(0);
                    String groupName = group.groupId();
                    List<OffsetFetchRequestData.OffsetFetchRequestTopics> topics = group.topics();
                    List<OffsetFetchRequestData.OffsetFetchRequestTopic> oldFormatTopics = null;
                    if (topics != null) {
                ...
                if (data.groups().isEmpty()) {
                    ...
                    List<OffsetFetchRequestData.OffsetFetchRequestTopic> oldFormatTopics = data.topics();
                    List<OffsetFetchRequestData.OffsetFetchRequestTopics> topics = null;
                    if (oldFormatTopics != null) {
                        topics = oldFormatTopics
                            .stream()
                            .map(t -> new OffsetFetchRequestData.OffsetFetchRequestTopics()
                                .setName(t.name())
                                .setPartitionIndexes(t.partitionIndexes()))
                            .collect(Collectors.toList());
                    }
    ...
    public Map<String, List<TopicPartition>> groupIdsToPartitions() {
        ...
        for (OffsetFetchRequestData.OffsetFetchRequestGroup group : data.groups()) {
    ...
    public Map<String, List<OffsetFetchRequestData.OffsetFetchRequestTopics>> groupIdsToTopics() {
        Map<String, List<OffsetFetchRequestData.OffsetFetchRequestTopics>> groupIdsToTopics =
    ...
    public List<String> groupIds() {
        return data.groups()
            .stream()
            .map(OffsetFetchRequestData.OffsetFetchRequestGroup::groupId)
            .collect(Collectors.toList());
    }
    ...
    public boolean isAllPartitionsForGroup(String groupId) {
        OffsetFetchRequestData.OffsetFetchRequestGroup group = data
            .groups()
            .stream()
