...
public class AccessTokenRetrieverFactory  {
    ...
    public static AccessTokenRetriever create(Map<String, ?> configs,
        ...
        URL tokenEndpointUrl = cu.validateUrl(SaslConfigs.SASL_OAUTHBEARER_TOKEN_ENDPOINT_URL);

        if (tokenEndpointUrl.getProtocol().toLowerCase(Locale.ROOT).equals("file")) {
            return new FileTokenRetriever(cu.validateFile(SaslConfigs.SASL_OAUTHBEARER_TOKEN_ENDPOINT_URL));
        } else {
            JaasOptionsUtils jou = new JaasOptionsUtils(jaasConfig);
            String clientId = jou.validateString(OAuthBearerLoginCallbackHandler.CLIENT_ID_CONFIG);
            String clientSecret = jou.validateString(OAuthBearerLoginCallbackHandler.CLIENT_SECRET_CONFIG);
            String scope = jou.validateString(OAuthBearerLoginCallbackHandler.SCOPE_CONFIG, false);
            ...
            return new HttpAccessTokenRetriever(clientId,
                ...
                cu.validateLong(SaslConfigs.SASL_LOGIN_RETRY_BACKOFF_MS),
                cu.validateLong(SaslConfigs.SASL_LOGIN_RETRY_BACKOFF_MAX_MS),
                cu.validateInteger(SaslConfigs.SASL_LOGIN_CONNECT_TIMEOUT_MS, false),
                cu.validateInteger(SaslConfigs.SASL_LOGIN_READ_TIMEOUT_MS, false));
        }
    }
