...
import com.tong.kafka.common.message.ListPartitionReassignmentsRequestData;
import com.tong.kafka.common.message.ListPartitionReassignmentsResponseData;
...
public class ListPartitionReassignmentsRequest extends AbstractRequest {
    ...
    public AbstractResponse getErrorResponse(int throttleTimeMs, Throwable e) {
        ...
        List<ListPartitionReassignmentsResponseData.OngoingTopicReassignment> ongoingTopicReassignments = new ArrayList<>();
        if (data.topics() != null) {
            for (ListPartitionReassignmentsRequestData.ListPartitionReassignmentsTopics topic : data.topics()) {
                ongoingTopicReassignments.add(
                        new ListPartitionReassignmentsResponseData.OngoingTopicReassignment()
                                ...
                                .setPartitions(topic.partitionIndexes().stream().map(partitionIndex ->
                                        new ListPartitionReassignmentsResponseData.OngoingPartitionReassignment().setPartitionIndex(partitionIndex)).collect(Collectors.toList()))
                );
            }
        }
