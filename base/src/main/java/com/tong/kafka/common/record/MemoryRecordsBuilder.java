 ...
 */
package com.tong.kafka.common.record;

import com.tong.kafka.common.header.Header;
import com.tong.kafka.common.message.LeaderChangeMessage;
import com.tong.kafka.common.message.SnapshotFooterRecord;
import com.tong.kafka.common.message.SnapshotHeaderRecord;
import com.tong.kafka.common.protocol.MessageUtil;
import com.tong.kafka.common.protocol.types.Struct;
import com.tong.kafka.common.utils.ByteBufferOutputStream;
...
public class MemoryRecordsBuilder implements AutoCloseable {
    ...
    private void appendWithOffset(long offset, boolean isControlRecord, long timestamp, byte[] allData) {
        try {
            if (isControlRecord != isControlBatch)
                throw new IllegalArgumentException("Control records can only be appended to control batches");
            ...
            if (magic > RecordBatch.MAGIC_VALUE_V1) {
                appendDefaultRecord(offset, timestamp, allData);
            } else {
                //todo
//                appendLegacyRecord(offset, timestamp, key, value, magic);
            }
    ...
    public void appendWithOffset(long offset, long timestamp, byte[] key, byte[] value, Header[] headers) {
        appendWithOffset(offset, false, timestamp, Utils.wrapNullable(key), Utils.wrapNullable(value), headers);
    }
    ...
    public void appendWithOffset(long offset, long timestamp, byte[] key, byte[] value) {
        appendWithOffset(offset, timestamp, Utils.wrapNullable(key), Utils.wrapNullable(value), Record.EMPTY_HEADERS);
    }
    ...
    public void append(long timestamp, byte[] key, byte[] value) {
        append(timestamp, Utils.wrapNullable(key), Utils.wrapNullable(value), Record.EMPTY_HEADERS);
    }
    ...
    private void appendDefaultRecord(long offset, long timestamp, byte[] allData) throws IOException {
        ensureOpenForRecordAppend();
        int offsetDelta = (int) (offset - baseOffset);
        long timestampDelta = timestamp - baseTimestamp;
        int sizeInBytes = DefaultRecord.writeTo(appendStream, offsetDelta, timestampDelta, allData);
        recordWritten(offset, timestamp, sizeInBytes);
    }
    ...
    public boolean hasRoomFor(long timestamp, byte[] key, byte[] value, Header[] headers) {
        return hasRoomFor(timestamp, Utils.wrapNullable(key), Utils.wrapNullable(value), headers);
    }
