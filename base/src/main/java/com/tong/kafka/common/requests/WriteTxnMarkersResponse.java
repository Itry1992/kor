...
public class WriteTxnMarkersResponse extends AbstractResponse {
    ...
    public WriteTxnMarkersResponse(Map<Long, Map<TopicPartition, Errors>> errors) {
        ...
        for (Map.Entry<Long, Map<TopicPartition, Errors>> markerEntry : errors.entrySet()) {
            Map<String, WriteTxnMarkersResponseData.WritableTxnMarkerTopicResult> responseTopicDataMap = new HashMap<>();
            for (Map.Entry<TopicPartition, Errors> topicEntry : markerEntry.getValue().entrySet()) {
                ...
                WriteTxnMarkersResponseData.WritableTxnMarkerTopicResult topic =
                    responseTopicDataMap.getOrDefault(topicName, new WriteTxnMarkersResponseData.WritableTxnMarkerTopicResult().setName(topicName));
    ...
    public Map<Long, Map<TopicPartition, Errors>> errorsByProducerId() {
        ...
        for (WriteTxnMarkersResponseData.WritableTxnMarkerResult marker : data.markers()) {
