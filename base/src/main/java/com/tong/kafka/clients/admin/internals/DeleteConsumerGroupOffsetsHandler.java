...
import com.tong.kafka.common.message.OffsetDeleteRequestData;
...
import com.tong.kafka.common.requests.OffsetDeleteRequest;
import com.tong.kafka.common.requests.OffsetDeleteResponse;
...
public class DeleteConsumerGroupOffsetsHandler extends AdminApiHandler.Batched<CoordinatorKey, Map<TopicPartition, Errors>> {
    ...
    public OffsetDeleteRequest.Builder buildBatchedRequest(int coordinatorId, Set<CoordinatorKey> groupIds) {
        ...
        final OffsetDeleteRequestData.OffsetDeleteRequestTopicCollection topics = new OffsetDeleteRequestData.OffsetDeleteRequestTopicCollection();
        partitions.stream().collect(Collectors.groupingBy(TopicPartition::topic)).forEach((topic, topicPartitions) -> topics.add(
            ...
            .setPartitions(topicPartitions.stream()
                .map(tp -> new OffsetDeleteRequestData.OffsetDeleteRequestPartition().setPartitionIndex(tp.partition()))
