...
import com.tong.kafka.common.metrics.stats.CumulativeSum
import com.tong.kafka.common.network.KafkaChannel.ChannelMuteEvent
import com.tong.kafka.common.network.{ChannelBuilder, ChannelBuilders, ClientInformation, KafkaChannel, ListenerName, NetworkSend, Send, Selector => KSelector}
import com.tong.kafka.common.protocol.ApiKeys
import com.tong.kafka.common.requests.{ApiVersionsRequest, RequestContext, RequestHeader}
import com.tong.kafka.common.security.auth.SecurityProtocol
import com.tong.kafka.common.utils.{KafkaThread, LogContext, Time}
import kafka.cluster.BrokerEndPoint
import kafka.metrics.KafkaMetricsGroup
import kafka.network.Processor.{IdlePercentMetricName, ListenerMetricTag, NetworkProcessorMetricTag}
...
private[kafka] class Processor(
  ...
  private[kafka] val metricTags: java.util.Map[String, String] = mutable.LinkedHashMap(
  ...
  protected[network] def sendResponse(response: RequestChannel.Response, responseSend: Send): Unit = {
    ...
    trace(s"Socket server received response to send to $connectionId, registering for write and sending payload: $response")
            ...
            if (header.apiKey == ApiKeys.SASL_HANDSHAKE && channel.maybeBeginServerReauthentication(receive,
              ...
              if (channel.serverAuthenticationSessionExpired(nowNanos)) {
               ...
               //构建请求，从receive 中获取buffer,然后根据APIKey进行反序列化
                ...
                //静默，取消对Selector OP_READ的监听，暂时不在接受新消息
                selector.mute(connectionId)
                //修改当前KafkaChannel的状态为处理请求中
