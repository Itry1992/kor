...
abstract class DelayedOperation(override val delayMs: Long,
  ...
  // Visible for testing
  private[kafka] val lock: Lock = lockOpt.getOrElse(new ReentrantLock)
   ...
   */
  private[kafka] def safeTryCompleteOrElse(f: => Unit): Boolean = inLock(lock) {
    if (tryComplete()) true
    else {
      f
   ...
   */
  private[kafka] def safeTryComplete(): Boolean = inLock(lock)(tryComplete())
