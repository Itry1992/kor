...
import com.tong.kafka.common.config.ConfigDef.{ConfigKey, ValidList}
import com.tong.kafka.common.config._
import com.tong.kafka.common.config.internals.BrokerSecurityConfigs
import com.tong.kafka.common.metrics.Sensor
import com.tong.kafka.common.network.ListenerName
import com.tong.kafka.common.record.{CompressionType, LegacyRecord, Records, TimestampType}
import com.tong.kafka.common.security.auth.SecurityProtocol
import com.tong.kafka.common.security.authenticator.DefaultKafkaPrincipalBuilder
import com.tong.kafka.common.utils.Utils
import com.tong.kafka.server.authorizer.Authorizer
import com.tong.kafka.server.common.MetadataVersion._
import com.tong.kafka.server.common.{MetadataVersion, MetadataVersionValidator}
import kafka.cluster.EndPoint
import kafka.message.{BrokerCompressionCodec, NoCompressionCodec, ProducerCompressionCodec}
...
//    "The server can also have a ZooKeeper chroot path as part of its ZooKeeper connection string which puts its payload under some path in the global ZooKeeper namespace. " +
...
//    s" Defaults to false if neither is set; when true, <code>$ZkClientCnxnSocketProp</code> must be set (typically to <code>com.tong.zookeeper.ClientCnxnSocketNetty</code>); other values to set may include " +
...
//  val ZkClientCnxnSocketDoc = "Typically set to <code>com.tong.zookeeper.ClientCnxnSocketNetty</code> when using TLS connectivity to ZooKeeper." +
...
//  /** ********* General Configuration ********** */
  ...
  val QueuedMaxRequestsDoc = "The number of queued requests allowed for payload-plane, before blocking the network threads"
  ...
  val LogDirDoc = "The directory in which the log payload is kept (supplemental for " + LogDirsProp + " property)"
  val LogDirsDoc = "A comma-separated list of the directories where the log payload is stored. If not set, the value in " + LogDirProp + " is used."
  ...
  val NumRecoveryThreadsPerDataDirDoc = "The number of threads per payload directory to be used for log recovery at startup and flushing at shutdown"
  ...
  val CreateTopicPolicyClassNameDoc = "The create topic policy class that should be used for validation. The class should " +
    "implement the <code>com.tong.kafka.server.policy.CreateTopicPolicy</code> interface."
  val AlterConfigPolicyClassNameDoc = "The alter configs policy class that should be used for validation. The class should " +
    "implement the <code>com.tong.kafka.server.policy.AlterConfigPolicy</code> interface."
  ...
  val UncleanLeaderElectionEnableDoc = "Indicates whether to enable replicas not in the ISR set to be elected as leader as a last resort, even though doing so may result in payload loss"
...
class KafkaConfig private(doLog: Boolean, configDef: ConfigDef, val props: java.util.Map[_, _])
  extends AbstractConfig(configDef, props, doLog) with Logging {

  def this(props: java.util.Map[_, _]) = this(true, KafkaConfig.configDef, KafkaConfig.populateSynonyms(props))

  def this(props: java.util.Map[_, _], doLog: Boolean) = this(doLog, KafkaConfig.configDef, KafkaConfig.populateSynonyms(props))

  def this(props: java.util.Map[_, _], doLog: Boolean, configDef: ConfigDef) = this(doLog, configDef, KafkaConfig.populateSynonyms(props))
  ...
  def usesSelfManagedQuorum = false
